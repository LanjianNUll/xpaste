...
- 🔗 **链接识别** - 自动识别 http://、https://、www. 开头的链接
- 🔍 **智能搜索** - 快速查找历史记录
- 🏷️ **自动分类** - 智能识别链接、图片、文本、文件
- 📅 **按日期筛选** - 今天/昨天/前天/自定义日期范围/全部时间查看

### 历史管理

- 📊 **历史记录** - 完整的历史管理页面，支持分页浏览
- 🔍 **多维度筛选** - 时间范围、内容类型、分类、关键字搜索
- 🗑️ **灵活删除** - 单条删除、批量删除、按类型删除、清空全部
- 📈 **统计信息** - 实时显示各类型记录数量统计
- 🔗 **链接预览** - 点击链接直接在浏览器中打开
- 🖼️ **图片预览** - 点击图片放大查看原图
...
2. **查看历史** - 点击托盘图标打开主窗口
3. **快速访问** - 按 Win+V 在光标位置弹出快捷窗口
4. **插入内容** - 点击任意历史记录，自动粘贴到当前输入框

### 快捷键

- **Win+V** - 覆盖 Windows 原生剪贴板历史并唤起快捷窗口（可自定义）
- **Ctrl+V** - 自动触发（由应用模拟）
...
1. 打开主窗口
2. 点击 "设置快捷键" 按钮
3. 输入新快捷键（格式：Win+V, Ctrl+Shift+C 等）
4. 重启应用生效
...
- 🔗 **链接** - http://、https://、www. 开头的网址（点击可直接打开）
- 📝 **文本** - 普通文本内容
- 🖼️ **图片** - 剪贴板中的图片（点击可放大预览）
- 📁 **文件** - 文件路径

### 历史管理页面

- **筛选功能**
  - 时间范围：全部/今天/昨天/前天/自定义范围
  - 分类筛选：链接/文本/图片/文件（支持多选）
  - 关键字搜索：实时搜索内容
  
- **批量操作**
  - 批量删除：选中多条记录一次性删除
  - 按类型删除：删除所有链接/文本/图片/文件
  - 清空全部：删除所有历史记录
...
- 🧩 页面与底层剪贴板 SDK 解耦
- ⚡ SQLite 服务端分页和低内存图片加载
- 🪟 默认使用 Win+V 唤起快捷窗口，支持 Esc 关闭

### v1.0.3

- ✨ 新增历史管理页面
- 🔗 新增链接类型自动识别（支持 http://、https://、www.）
- 🔗 链接点击直接在浏览器中打开
- 📊 新增按分类统计功能
- 🗑️ 新增按类型删除功能（链接/文本/图片/文件）
- 🔍 新增分类筛选功能
- 🖼️ 图片点击放大预览
- 📅 时间筛选支持自定义日期范围和全部时间
- 🌐 分页组件中文化
